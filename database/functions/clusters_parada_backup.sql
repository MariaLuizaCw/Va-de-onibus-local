...
CREATE OR REPLACE FUNCTION fn_analisar_clusters_linha(
    ...
    p_percentil_hull numeric DEFAULT 0.9,  -- percentil de pontos para o convex hull (0.8 = 80%, 0.9 = 90%)
    p_velocidade_parado_ms numeric DEFAULT 1.0  -- velocidade máxima para considerar parado (m/s). 1.0 = ~3.6 km/h
)
RETURNS TABLE (
    cluster_id integer,
    num_paradas bigint,
...
BEGIN
        ...
        SELECT
            ...
            CASE 
                ...
                WHEN (ST_Distance(
                        ST_SetSRID(ST_MakePoint(o.lon, o.lat), 4326)::geography,
                        ST_SetSRID(ST_MakePoint(o.lon_prev, o.lat_prev), 4326)::geography
                     ) / EXTRACT(EPOCH FROM (o.ts - o.ts_prev))) < p_velocidade_parado_ms
                THEN 1 ELSE 0
            END AS parado,
            LAG(
                CASE 
    ...
    -- Calcula centróide de cada cluster para usar no cálculo de distâncias
    centroides AS (
        SELECT
            c.cid,
            ST_Centroid(ST_Collect(c.geom::geometry))::geography AS centroide_geog
        FROM clusters c
    ...
    pontos_com_distancia AS (
        SELECT
            c.*,
            ST_Distance(c.geom::geography, cent.centroide_geog) AS dist_ao_centroide
        FROM clusters c
        INNER JOIN centroides cent ON c.cid = cent.cid
    ),

    -- Percentil das distâncias (raio que contém X% dos pontos, configurável)
    raio_cluster AS (
        SELECT
            cid,
            PERCENTILE_CONT(p_percentil_hull) WITHIN GROUP (ORDER BY dist_ao_centroide) AS raio_percentil_metros
        FROM pontos_com_distancia
        GROUP BY cid
    ),

    -- Filtra apenas pontos dentro do percentil (descarta outliers)
    pontos_filtrados AS (
        SELECT pcd.*
        FROM pontos_com_distancia pcd
        INNER JOIN raio_cluster rc ON pcd.cid = rc.cid
        WHERE pcd.dist_ao_centroide <= rc.raio_percentil_metros
    ),

    -- Convex hull dos pontos filtrados (polígono que envolve 90% dos pontos)
    hull_cluster AS (
        SELECT
            cid,
            ST_ConvexHull(ST_Collect(geom::geometry))::geography AS geom_hull
    ...
    resultado_temp AS (
        SELECT
            ...
            -- Total de pontos GPS usados para formar o cluster
            SUM(c.num_pontos)::bigint AS total_pontos_gps,
            -- Raio do percentil (para referência)
            ROUND(rc.raio_percentil_metros::numeric, 2) AS max_distance_metros,
            ...
            hc_hull.geom_hull AS geom_hull_p80
        FROM clusters c
        LEFT JOIN horas_cluster hc ON c.cid = hc.cid
        LEFT JOIN duracao_cluster dc ON c.cid = dc.cid
        LEFT JOIN raio_cluster rc ON c.cid = rc.cid
        LEFT JOIN hull_cluster hc_hull ON c.cid = hc_hull.cid
        WHERE c.cid IS NOT NULL
        GROUP BY c.cid, hc.hora_mediana, dc.mediana_duracao_segundos, rc.raio_percentil_metros, hc_hull.geom_hull
    ),
    ...
    classificacao_inicial AS (
        SELECT
            ...
            rt.primeira_parada,
            rt.ultima_parada,
            rt.tempo_total_parado_segundos,
            rt.tempo_total_parado_minutos,
            rt.mediana_duracao_minutos,
            rt.lat_cluster,
            rt.lon_cluster,
            rt.total_pontos_gps,
            rt.max_distance_metros,
            rt.hora_mediana_cluster,
            rt.mediana_duracao_minutos_temp,
            rt.geom_coletado,
            rt.geom_hull_p80,
            CASE 
    ...
    -- Usa o convex hull dos pontos dentro do percentil 80
    clusters_com_buffer AS (
        SELECT
            ci.cid,
            ci.geom_hull_p80 AS geom_cluster
